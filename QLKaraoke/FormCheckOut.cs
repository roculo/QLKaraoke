...
namespace QLKaraoke
{
    public partial class FormCheckOut : Form
    {
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            tongtien = ((double)tienfood + (double)tienroom + (double)tienservice)*discount;

            lbTotal.Text = "" + tongtien.ToString("C");
            //Close
            DatabaseConnect.myConn.Close();
            //}
            //catch
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui Long Chon Phong Thanh Toán");
                return;
            }
        ...
        {// Set Invoice Total:
         // Draw line:
            ...
            if(discount==0.9)
            {
                int xInvoiceT = AmountPosition - (int)g.MeasureString("Invoice Total", InvoiceFont).Width;
                CurrentY = CurrentY + InvoiceFontHeight;
                g.DrawString("Discount", InvoiceFont, RedBrush, 300, CurrentY);
                string InvoiceD = "10%";
                int xInvoiceD = xRightEdg - (int)g.MeasureString(InvoiceD, InvoiceFont).Width;
                g.DrawString(InvoiceD, InvoiceFont, BlackBrush, xFreight, CurrentY);
            }
